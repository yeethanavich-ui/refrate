...
export default function AdminPage() {
  ...
  if (!userId || !profile?.is_admin) {
    return <p className="text-steel-400">You don&apos;t have access to this page.</p>;
  }
  ...
  return (
    <div className="space-y-10">
      ...
      <section className="space-y-4">
        <h2 className="font-display text-xl text-steel-100">All reviews ({reviews.length})</h2>
        {loading ? (
          <p className="text-steel-400">Loading...</p>
        ) : (
          <ul className="space-y-3">
            {reviews.map((r) => (
              <li key={r.id} className="rounded-lg border border-piste-700 bg-piste-900 p-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-steel-100">
                      {refNameById.get(r.ref_id) || `Referee #${r.ref_id}`} — {r.rating}★
                    </p>
                    <p className="text-sm text-steel-400">
                      by {r.reviewer_name || "Unknown"} · {new Date(r.created_at).toLocaleDateString()}
                    </p>
                    {r.comment && <p className="mt-1 text-steel-300">{r.comment}</p>}
                  </div>
                  <button
      ...
      <section className="space-y-4">
        <h2 className="font-display text-xl text-steel-100">All referees ({referees.length})</h2>
        <ul className="space-y-2">
          {referees.map((ref) => (
            <li key={ref.id} className="flex items-center justify-between rounded-lg border border-piste-700 bg-piste-900 p-3">
              <span className="text-steel-100">
                {ref.name} <span className="text-sm text-steel-400">({ref.region})</span>
              </span>
              <button
